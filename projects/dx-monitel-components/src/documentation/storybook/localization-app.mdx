...
#### 3. Добавить файлы **autolocale.json**, **dx.en.json** и **dx.ru.json** в папку **./src/assets/i18n**

Пример словаря **autolocale.json**
...
setLocalization().subscribe(value => {
  const langId = i18next.language || 'ru';
  Object.assign(environment, value);
  platformBrowserDynamic([{provide: LOCALE_ID, useValue: langId}]).bootstrapModule(AppModule)
    .catch(err => console.log(err));
});

function setLocalization(): Observable<any> {
  return fetchToObservable<any>('assets/i18n/autolocale.json').pipe(
