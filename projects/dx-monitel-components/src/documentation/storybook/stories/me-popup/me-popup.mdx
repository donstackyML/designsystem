...
- [Попап в формате диалогового окна (средний размер)](#попап-в-формате-диалогового-окна-средний-размер)
- [Попап в формате диалогового окна (большой размер)](#попап-в-формате-диалогового-окна-большой-размер)
...
Добавление заголовка осуществляется в поле `title` компонента, поле `showTitle` должно быть `true`
(можно его не прописывать, т.к. оно по умолчанию `true`).
...
Чтобы добавить иконку, описание и кнопку more нужно сделать кастомный темплейт с использованием `dxTemplate`, также добавить параметр `titleTemplate="title"`.
В темплейте использовать классы `me-popup-header`, для отображения заголовка с описанием под ним использовать класс `me-sidepage-title`.
...
Чтобы получить внутри окна скролл из дизайн-системы необходимо обернуть контент в компонент `dx-scroll-view`.

<Canvas of={popupStories.ScrollableContent} />
...
<Canvas of={popupStories.ScrollableContentWithButtons} />
...
## Попап в формате диалогового окна (средний размер)

Для корректного отображения отступов установите свойство [isDialog]="true"

<Canvas of={popupStories.DialogMedium} />

<Source
  code={`
<dx-popup mePopup titleTemplate="title" [showTitle]="false" [isDialoge]="true"
...
## Попап в формате диалогового окна (большой размер)
...
<Source
  code={`
<dx-popup mePopup titleTemplate="title" [showTitle]="true" [isDialog]="true"
      <dxi-toolbar-item template="confirmButton" toolbar="bottom"> </dxi-toolbar-item>
      <dxi-toolbar-item template="cancelButton" toolbar="bottom"> </dxi-toolbar-item>
      <div *dxTemplate="let data of 'content'">
        <dx-scroll-view width="100%" height="100%">
...
Предлагается вместо кастомного флага находить попап помеченный шаблонной переменной с помощью `@ViewChild` и закрывать нужный используя встроенные методы DX `show()` и `hide()`. Например:
...
<Source
  code={`<dx-button text="Open popup" (onClick)="showPopup()"></dx-button>`}
  dark
  language="html"
/>
...
<Canvas of={popupStories.SizeLarge} />
...
<Source
  code={`<dx-popup mePopup [showTitle]="true" title="Заголовок" size="large">
    ...
    <div *dxTemplate="let data of 'confirmButton'">
      <dx-button meButton size="large" text="Принять" buttonType="default"></dx-button>
    </div>
    <div *dxTemplate="let data of 'cancelButton'">
