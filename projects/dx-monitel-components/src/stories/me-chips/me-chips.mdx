...
<Meta
  ...
  parameters={{
    docs: {
      description: {
        component: 'Компоненты MeChipsContainer и MeChip с поддержкой удаления чипов и улучшенными стилями. Чипы становятся активными при клике.',
      },
    },
  }}
  of={ChipsStories}
...
Компоненты `MeChip` и `MeChipsContainer` имеют обновленные стили:

- Высота чипа: 32px
- Скругление углов: 16px
- Цвета фона и текста соответствуют макету
- Иконка удаления с использованием `MeIconComponent`
- Стили для различных состояний (активный, отключенный, сфокусированный)
