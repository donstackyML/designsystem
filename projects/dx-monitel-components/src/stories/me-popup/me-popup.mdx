...
Предлагается вместо кастомного флага находить попап помеченный шаблонной переменной с помощью <code>@ViewChild</code> и закрывать нужный используя встроенные методы DX <code>show()</code> и <code>hide()</code>. Например:
